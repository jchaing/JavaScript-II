...
// Problem 1
// The marketing department would like all of the runner's email addresses to send out newsletters
let allEmails = runners.map(runner => runner.email);
console.log(allEmails);

// Problem 2
// Create a list of runners that donated $200 or more so that they can receive a gift
let donateHundred = runners.filter(runner => runner.donation >= 200);
console.log(donateHundred);

// Problem 3
// All runners will be entered in a raffle to receive a prize. Pick a random runner to receive the prize
let randomID = Math.floor(Math.random() * (runners[runners.length - 1].id - runners[0].id) + runners[0].id);
let randomRunner = runners.filter(runner => runner.id === randomID);
